# devops-infra/action-format-hcl

GitHub Action automatically formatting all HCL and TF files

Hardened by [Chainguard](https://www.chainguard.dev) from the upstream action at [https://github.com/devops-infra/action-format-hcl](https://github.com/devops-infra/action-format-hcl).

## Versions

| Version | Tag | Upstream commit |
|---------|-----|-----------------|
| v1.1.1 | [`v1.1.1`](https://github.com/chainguard-actions/devops-infra-action-format-hcl/tree/v1.1.1) | [`749b8c7`](https://github.com/devops-infra/action-format-hcl/commit/749b8c71b17ad07a4a45f23af4c9d36eafc61f7f) |

## Privacy

This Action contacts Chainguard's licensing server to verify authorization. Connection metadata (IP address, GitHub repository identifier, timestamp, and any metadata encoded in the auth token) is transmitted to Chainguard, Inc. even if authorization is denied in accordance with our [Privacy Notice](https://www.chainguard.dev/legal/privacy-notice)
